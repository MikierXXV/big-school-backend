...
import { RefreshTokenStatus } from '../../../domain/value-objects/refresh-token.value-object.js';
...
export class RefreshSessionUseCase {
  ...
  public async execute(
    ...
    // 2. Obtener hash del token para buscar en BD
    ...
    // 4. CRÍTICO: Verificar si el token ya fue usado (ROTATED)
    // if (existingToken.status === RefreshTokenStatus.ROTATED) {
    //   // ¡ALERTA DE SEGURIDAD! Reuso detectado
    //   this.deps.logger.error('SECURITY: Refresh token reuse detected', {
    ...
    //   );
    //   if (familyRootId) {
    //     await this.deps.refreshTokenRepository.revokeFamily(familyRootId);
    //   }
    //
    ...
    // 5. Verificar si el token fue revocado
    ...
    // 7. Verificar que el usuario existe y está activo
    // const user = await this.deps.userRepository.findById(
    //   UserId.create(existingToken.userId)
    // );
    // if (!user) {
    ...
    // 9. Generar nuevo refresh token (ROTACIÓN)
    // const newTokenId = this.deps.uuidGenerator.generate();
    // const newRefreshToken = RefreshToken.createRotated(
    //   await this.deps.tokenService.generateRefreshToken({
    //     userId: user.id.value,
    //     tokenId: newTokenId,
    ...
    //   existingToken,
    //   now,
    //   this.deps.dateTimeService.addSeconds(RefreshToken.VALIDITY_SECONDS)
    // );

    // 10. Marcar token anterior como ROTATED
    // await this.deps.refreshTokenRepository.updateStatus(
    //   existingToken.tokenId,
    //   RefreshTokenStatus.ROTATED
    // );

    // 11. Almacenar nuevo refresh token
    ...
    throw new Error('RefreshSessionUseCase not implemented');
  }
}
